...
router.get("/random-movies-by-genres", (req, res) => {
  ...
  if (genreIds.length === 0) {
    return res
      .status(400)
      .json({ error: "Please provide at least one genre ID." });
  }
  ...
  connection.query(getRandomMoviesQuery, genreParams, (err, results) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "An error occurred while fetching random movies." });
    }
    ...
    if (!results || results.length === 0) {
      return res
        .status(404)
        .json({ message: "No movies found for the provided genres." });
    }
